...
# Decide later if we want to use NSIDC data instead. The NSIDC at 1 km is of better spatial resolution.
# ftp://sidads.colorado.edu/DATASETS/NOAA/G02186/shapefiles

# test <- st_read("~/Downloads/masie_ice_r00_v01_2016163_1km/masie_ice_r00_v01_2016163_1km.shp") %>%
#   st_transform("+proj=stere +lat_0=90 +lat_ts=70 +lon_0=-45 +k=1 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs") %>%
#   st_crop(c(xmin = -2121762.1561, ymin = -3172643.4391, xmax = -14302.5504, ymax = -1331922.1268)) %>%
#   st_transform(st_crs(shapes2)) %>%
#   st_cast("LINESTRING") %>%
#   st_crop(c(xmin = -65, ymin = 66.5, xmax = -50, ymax = 72)) %>%
#   st_intersection(baffin_interior)
